...
btn_set_subject_message = telebot.types.InlineKeyboardButton('تعيين الموضوع والكليشه', callback_data='set_subject_message')
...
btn_delete_email = telebot.types.InlineKeyboardButton('مسح ايميل معين', callback_data='delete_email')
...
btn_delete_klishes = telebot.types.InlineKeyboardButton('شرح البوت' , url='https://t.me/mmakkkaaas/3')
...
btn_one_day = telebot.types.InlineKeyboardButton('1️⃣ يوم', callback_data='duration_1_day')
btn_one_week = telebot.types.InlineKeyboardButton('7️⃣ اسبوع', callback_data='duration_1_week')
btn_one_month = telebot.types.InlineKeyboardButton('🗓️ شهر', callback_data='duration_1_month')
...
def start(message):
    ...
    if user_id in allowed_users:
        ...
        bot.reply_to(message, 'اهلا بك عزيزي في بوت رفع خارجي', reply_markup=keyboard)
    else:
        bot.reply_to(message, 'انت غير مشترك في البوت للاشتراك في البوت : @G_E_N')
...
def show_admin_commands(message):
    if str(message.from_user.id) == '5187003844':  
